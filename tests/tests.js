function studentrecord_tests(){
	...
	function testCallback(input){
		if(input === undefined){
			throw('error 1: click callback for delete button did not provide an argument of the StudentRecord that was clicked.  returned undefined');
		}
		if(input.constructor !== StudentRecord){
			throw('error 2: click callback for delete button did not provide an argument of the StudentRecord that was clicked.  returned ' + JSON.stringify( input ));
		}
		testVal = true;
	}
	...
	try{
		...
		if(dom.prop('tagName')!=='TR'){
			throw('error 3: render() did not return a table row (TR)');
		}
		var children = dom.find('td');
		if( children.length !== 4){
			throw('error 4: render() should return a TR with 4 TDs, found ' + children.length);
		}
		if( children[0].innerHTML !== 'name'){
			throw(`error 5: constructor was given name of "name", first TD should have had "name".  It had ${children[0].innerHTML}`);
		}
		if( children[1].innerHTML !== 'course'){
			throw(`error 6: constructor was given name of "course", first TD should have had "course".  It had ${children[1].innerHTML}`);
		}
		if( children[2].innerHTML !== '100'){
			throw(`error 7: constructor was given name of 100, first TD should have had 100.  It had ${children[2].innerHTML}`);
		}
		var button = $(children[3]).find('button');
		if( button.length !== 1){
			throw('error 8: 4th TD should have had a button in it, could not find one');
		}
		if( button.text() !== 'DELETE' ){
			throw('error 9: 4th TD button should have had text of DELETE, it had ' + button.text());
		}
	} catch( error ){
	...
	try{
		...
		if(!testVal ){
			throw('error 10:callback function passed into constructor was not called upon calling handleDelete.');
		}
		if( document.querySelector('body').contains(dom2[0])){
			throw('error 11:dom elements for StudentRecord were not removed with calling handleDelete');
		}
		testVal = false;
		$("body").append(dom);
		button.click();
		...
		if(!testVal ){
			throw('error 12:callback function passed into constructor was not called upon clicking the delete button.');
		}
		if( document.querySelector('body').contains(dom[0])){
			throw('error 13:dom elements for StudentRecord were not removed with the click of the delete button');
		}
	...
	try{
		...
		if(data === undefined ){
			throw('error 14:callback function passed into constructor was not called upon calling handleDelete.');
		}
		if(data.constructor !== Object){
			throw('error 15:returned object from getData was not a native object, it was of type '  + data.constructor.name);
		}
		if(data.name === undefined){
			throw('error 16:returned object from getData did not have the key "name"');
		}
		if(data.name !== 'John'){
			throw('error 17:returned object from getData did not have correct value for the key "name", expected "John" but got ' + data.name);
		}
		if(data.course === undefined){
			throw('error 18:returned object from getData did not have the key "course"');
		}
		if(data.course !== 'Math'){
			throw('error 19:returned object from getData did not have correct value for the key "course", expected "Math" but got ' + data.course);
		}
		if(data.grade === undefined){
			throw('error 20:returned object from getData did not have the key "grade"');
		}
		if(typeof data.grade !== 'number'){
			throw('error 21:returned object from getData did not return the appropriate datatype.  expected number but got ' + typeof data.grade);
		}
		if(data.grade !== 100){
			throw('error 22:returned object from getData did not have correct value for the key "grade", expected 100 but got ' + data.grade);
		}
	} catch( error ){
	...
	try{
		...
		if(result!==false){
			throw('error 23:update method did not return false when given an illegal field name of "mooo", gave ' + result + ' instead');
		}
		var dom = student.render();
		result = student.update('name','Jack');
		data = 	student.getData();
		if(result!==true){
			throw('error 24:update method did not return true when given a valid value to change');
		}
		if(data.name !== 'Jack'){
			throw('error 25:update method did not change the student name to "Jack", it is '+ data.name);
		}
		if(dom.find('td')[0].innerHTML !== 'Jack'){
			throw('error 26:update method did not update the name dom element of the student, the value in the dom element should have been "Jack" but was '+dom.find('td')[0].innerHTML);
		}
...
function model_tests(){
	...
	try{
		...
		if(id !== 1){
			throw('error 27:Exepected 1 as first ID from getNextID, got'+id);
		}
		id = testModel.getNextID();
		if(id !== 2){
			throw('error 28:Expected 2 as next ID from getNextID, got '+id);
		}
	} catch( error ){
	...
	try{
		...
		if(count !== 1){
			throw('error 29:added a student, expected 1 as returned count, got'+count);
		}
		var count2 = testModel.add('student2', 'course2', 51);
		if(count2 !== 2){
			throw('error 30:added a student, expected 2 as returned count, got'+count);
		}
	} catch( error ){
		displayMessage(['error with Model getNextID: ',error],'error');
		return false;
	...
	try{
		...
		if(!Array.isArray( studentList )){
			throw('error 31:method should return an array, returned a ' + typeof studentList)
		}
		if(studentList.length!==2){
			throw('error 32:There should be 2 items in the array, there were '+studentList.length);
		}
		if(studentList[0].constructor !== StudentRecord){
			throw('error 33:method must return an array of StudentRecord, the contained pieces were made by a ' + studentList.constructor)
		}
		if(studentList[0].getData().name !== 'student1'){
			throw('error 34:first student added did not have a name of "student1", had '+ studentList[0].getData().name);
		}
		if(studentList[1].getData().name !== 'student2'){
			throw('error 35:second student added did not have a name of "student1", had '+ studentList[1].getData().name);
		}
		if(studentList[0].getData().course !== 'course1'){
			throw('error 36:first student added did not have a course of "course1", had '+ studentList[0].getData().course);
		}
		if(studentList[1].getData().course !== 'course2'){
			throw('error 37:second student added did not have a course of "course2", had '+ studentList[1].getData().course);
		}
		if(studentList[0].getData().grade !== 50){
			throw('error 38:first student added did not have a grade of 50, had '+ studentList[0].getData().grade);
		}
		if(studentList[1].getData().grade !== 51){
			throw('error 39:second student added did not have a grade of 51, had '+ studentList[1].getData().grade);
		}
	} catch( error ){
	...
	try{
		...
		if(result!==true){
			throw('error 40:Should have returned true after successfully removing student, returned'+result);
		}
		var newList = testModel.getAllStudents();
		if(newList.length !== 1){
			throw('error 41:There should be 1 student left, but count is '+ newList.length);
		}
		...
		if(result!==false){
			throw('error 42:Tried to remove "haha" from student list, should have gotten false, but got '+result);
		}
		result = testModel.remove(studentListCopy[0]);
		if(result!==false){
			throw('error 43:Tried to remove a student that was already removed.  Should have gotten false, but got '+ result);
		}
	} catch( error ){
	...
	try{
		...
		if(result !== studentListCopy[1]){
			throw('error 44:Tried to fetch student by name "student2", should have gotten '+JSON.stringify(studentListCopy[1])+', but got ' + result);
		}
		result = testModel.getStudentByField('grade',51);
		if(result !== studentListCopy[1]){
			throw('error 45:Tried to fetch student by grade 52, should have gotten '+JSON.stringify(studentListCopy[1])+', but got ' + result);
		}
		result = testModel.getStudentByField('splunge', 'student2');
		if(result !== -1){
			throw('error 46:Tried to find a student by a field that does not exist, expected -1, but got '+result);
		}
		result = testModel.getStudentByField('name','chuck');
		if(result !== -1){
			throw('error 47:Tried to find a student by a name called "chuck" that does not exist, should have gotten -1, but got '+result);
		}
	} catch( error ){
	...
	try{
		...
		if(result!==51){
			throw('error 48:Tried to get average of 1 student with a grade of 51.  Expected average of 51.  Got '+ result);
		}
		testModel.add('student3', 'course3', 51);
		result = testModel.calculateGradeAverage();
		if(result!==51){
			throw('error 49:Tried to get average of 2 students both with a grade of 51.  Expected average of 51.  Got '+ result);
		}
		testModel.add('student4', 'course4', 100);
		result = testModel.calculateGradeAverage();
		if(result!==(51+51+100)/3){
			throw('error 50:Tried to get average of 51, 51, and 100.  Expected average of 67.33(repeating).  Got '+ result);
		}		
	} catch( error ){
...
function sgt_tests(){
	...
	try{
		...
		if(i===eventData.click.length){
			throw('error 51:Could not find handleAdd as a click handler on the add button');
		}
		...
		if(i===eventData.click.length){
			throw('error 52:Could not find handleAdd as a click handler on the add button');
		}
	} catch( error ){
	...
	try{
		...
		if(elementSelectors.nameInput.val()!==''){
			throw('error 53:clearInputs didn\'t clear the name input');
		}
		if(elementSelectors.courseInput.val()!==''){
			throw('error 54:clearInputs didn\'t clear the course input');
		}
		if(elementSelectors.gradeInput.val()!==''){
			throw('error 55:clearInputs didn\'t clear the grade input');
		}
	...
	try{
		...
		if(elementSelectors.nameInput.val()!==''){
			throw('error 56:handleCancel didn\'t clear the name input');
		}
		if(elementSelectors.courseInput.val()!==''){
			throw('error 57:handleCancel didn\'t clear the course input');
		}
		if(elementSelectors.gradeInput.val()!==''){
			throw('error 58:handleCancel didn\'t clear the grade input');
		}
		displayMessage('handleCancel cleared all inputs','message');
		elementSelectors.nameInput.val('test');
		elementSelectors.cancelButton.click();
		if(elementSelectors.nameInput.val()!==''){
			throw('error 59:cancel button click did not clear the inputs');
		}		
	} catch( error ){
	...
	try{
		...
		if(text !== '00'){ //there are 2 display areas
			throw('error 60:expected to see 0 in both average display areas, got ' + text.substr(0,text.length/2))
		}
		...
		if(text !== '5050'){ //there are 2 display areas
			throw('error 61:expected to see 50 in both average display areas, got ' + text.substr(0,text.length/2))
		}
	} catch( error ){
	...
	try{
		...
		if(trs.length!==2){
			throw('error 62:Expected to see 2 student records displayed, got ' + trs.length);
		}
		var firstName = trs.eq(0).find('td:first-child').text()
		if(firstName!=='dude'){
			throw('error 63:Expected first student\'s display name to be "dude", found '+firstName)
		}
		var secondName = trs.eq(1).find('td:first-child').text()
		if(secondName!=='dudette'){
			throw('error 64:Expected second student\'s display name to be "dudette", found '+firstName)
		}
		testSGT.displayAllStudents();
		var trs2 = elementSelectors.displayArea.find("tr");
		if(trs2.length!==2){
			if(trs2.length === 4){
				throw('error 65:Expected to see 2 student records displayed, got 4.  Did you forget to clear the display area?');
			}
			throw('error 66:Expected to see 2 student records displayed, got ' + trs2.length);
		}
		var secondGrade = trs.eq(1).find('td:nth-child(3)').text()
		if(secondGrade!=='100'){
			throw('error 67:Expected second student\'s display grade to be 100, found '+secondGrade);
		}
	} catch( error ){
	...
	try{
		...
		testSGT.handleAdd();
		if(elementSelectors.nameInput.val()!==''){
			throw('error 68:handleAdd did not invoke clearInputs or did not clear the inputs after add');
		}
		...
		if(post_students.length !== pre_students.length + 2){
			throw('error 69:clicking on the add button did not save the data to a new student in the model');
		}
		var text = elementSelectors.averageArea.text();
		if(text !== '7575'){ //there are 2 display areas
			throw('error 70:expected to see 75 in both average display areas, got ' + text.substr(0,text.length/2))
		}		
	} catch( error ){
...
function displayMessage(message, type='error'){
	...
	if(type==='error'){
		console.error('error in code, see below for details.  Check under the section that ends in _tests:');
		console.trace(wholeMessage);
	} else {
...
function testMethod( object, method ){
	try{
		if(object[method] === undefined){
			throw('error : missing method '+method+' in ' + object.constructor.name);
		}
	}
	catch (error){
